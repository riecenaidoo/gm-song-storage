...
public class PlaylistSongsController {
  ...
  public PlaylistSongsController(PlaylistSongQueryRepository playlistSongs,
   ...
   */
  @PostMapping
  public ResponseEntity<PlaylistSongResponse> createPlaylistSong(@PathVariable int playlistId,
                                                                 @RequestBody PlaylistSongsCreateRequest request) {
    ...
    URI resource = ServletUriComponentsBuilder.fromCurrentRequestUri()
                                              .path(String.format("/%d", playlistSong.getId())).build().toUri();
  ...
  public ResponseEntity<PlaylistSongResponse[]> readPlaylistSongs(@PathVariable int playlistId) {
    ...
    PlaylistSongResponse[] response = playlistSongs.stream().map(PlaylistSongResponse::new)
                                                   .toArray(PlaylistSongResponse[]::new);
    return ResponseEntity.ok(response);
  }

  @DeleteMapping("/{id}")
  public ResponseEntity<Void> removePlaylistSong(@PathVariable int playlistId,
                                                 @PathVariable int id) {
    Playlist playlist = playlists.findById(playlistId)
                                 .orElseThrow(() -> new ResourceNotFoundException(Playlist.class, playlistId));
    PlaylistSong playlistSong = playlistSongs.findById(id)
                                             .orElseThrow(() -> new ResourceNotFoundException(PlaylistSong.class, id));
    if (!playlist.getId().equals(playlistSong.getPlaylist().getId())) {
      throw new SubresourceMismatchException(Playlist.class, playlist.getId(),
