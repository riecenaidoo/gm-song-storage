...
@UnitTest(PlaylistSongsController.class)  // In Spring, this is a Slice test, but that concept isn't universal - I think.
class PlaylistSongsControllerTest {

  // Mock Dependencies

  @MockitoBean
  private PlaylistSongQueryRepository playlistSongs;
  ...
  void createPlaylistSong() throws Exception {
    ...
    */
    ...
    when(songService.create(request.toCreate())).thenAnswer(invocation -> {
      Song song = invocation.getArgument(0);
      return songMother.setId(song);
    });
    when(service.create(any(PlaylistSong.class))).thenAnswer(invocation -> {
      PlaylistSong playlistSong = invocation.getArgument(0);
      return EntityMother.setId(playlistSong, id);
    });

    // When
    mvc.perform(post("/api/v2/playlists/{playlist_id}/songs", playlist.getId(), id)
  ...
  void readPlaylistSongs() throws Exception{
    // Given
    Playlist playlist = new PlaylistMother(random).withIds().get();
    PlaylistSong[] allPlaylistSongs = new PlaylistSongMother(random).withPlaylists(() -> playlist)
                                                                    .get(random.nextInt(1, 101))
                                                                    .toArray(PlaylistSong[]::new);
    PlaylistSongResponse[] expectedResponse = Arrays.stream(allPlaylistSongs).map(PlaylistSongResponse::new)
  ...
  void removePlaylistSong() throws Exception {
    ...
    // When
    mvc.perform(delete("/api/v2//playlists/{playlist_id}/songs/{id}",
                       playlistId, id))
       // Then
