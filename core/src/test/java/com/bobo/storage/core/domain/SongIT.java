...
/**
 * Testing {@code Song} integrations with the web. Makes live calls on testing URLs to verify things work as expected
 * in the wild.
 */
...
class SongIT {
  ...
  /**
   * @return known redirecting URLs to test against. Note that these are typically URLs are typically pointing at a
   * temporary that are going to eventually be removed and requesting you redirect to the new permanent location - so
   * they would need to be verified. It is pointless writing a test to validate them, as that is what the method we are
   * testing is trying to do. We can just do an assumption assertion that these are 3xx requests, however.
   */
  protected static Stream<RedirectingURL> redirectingUrls() {
    return Stream.of(
            // Song in 'prod' db that does not get picked up.
            new RedirectingURL("https://dzr.page.link/iao9CHJgGQA2gDFn9",
   ...
   *                       When populating this value, do not use a browser that may send additional headers and cookies.
   *                       Use {@code curl --head} locally.
   */
