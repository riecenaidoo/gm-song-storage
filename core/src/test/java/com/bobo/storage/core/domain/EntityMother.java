...
public interface EntityMother<T extends DomainEntity> extends Mother<T> {
   ...
   * <p>
   * The signature to be used when the mocking is not concerned with what the {@code id} is,
   * just that it exists.
   */
  T setId(T entity);
   ...
   * Assign an {@code id} to the {@code Entity}.
   * <p>
   * To signature to be used when you need control over the mocked {@code id}.
   */
